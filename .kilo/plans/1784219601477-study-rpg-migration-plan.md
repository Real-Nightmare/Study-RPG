...
| Backend API | NestJS 10 (existing codebase) | MonkeysCloud |
| Database | PostgreSQL | MonkeysCloud (included) |
| Cache | Redis | MonkeysCloud (included) |
| Real-time | Socket.io | MonkeysCloud (NestJS WebSocket) |
...
### 3. Backend AI Feature Simplification (for D1/No-Qdrant)
...
1. **MonkeysCloud stability**: New platform, free tier may change. Need fallback plan (Render or SnapDeploy).
2. **Google OAuth**: Need to create Google Cloud project and configure OAuth consent screen — do you have existing credentials?
3. **OpenRouter API key**: Need a key for AI features — have one already?
4. **Custom domain**: Will you use a custom domain for Cloudflare Pages, or the default `*.pages.dev`?
5. **Data migration**: Any existing Studyield data to preserve, or starting fresh?

---

## Risks

- **MonkeysCloud is new**: Platform may have bugs or change terms. Have Render as fallback.
